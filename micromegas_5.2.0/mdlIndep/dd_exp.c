...
#ifdef CRESST_III
{
  ...
  for(int i=0;i<100;i++)
  {  Mcdm=mMin*pow(mMax/mMin, (i+0.5)/100.);
   csTab4[i]=CRESST_III_SDn_90(Mcdm)/CRESST_III_90(Mcdm);
   csTab3[i]=csTab2[i]/csTab1[i];
  }     
   displayPlot("SD/SI","M[GeV]", mMin,mMax,1,2
                    ,"CRESST_III",100, csTab4,NULL
                    ,"micrOMEGAs", 100, csTab3,NULL 
  );
...
#ifdef SD_LIMITS
{
   ...
   double dnde[100];
   csTest=PICO60_SDp_90(Mcdm)*1E36;
   nucleusRecoilCS(Maxwell,19,Z_F,J_F19,SxxF19EFT,0,0,csTest,0,dNdE);
   for(int i=0;i<100;i++) dnde[i]=dNdERecoil( 0.4*(i+0.5)+4, dNdE);
   csTest=PICO60_90(Mcdm)*1E36;  
   nucleusRecoilCS(Maxwell,19,Z_F,J_F19,SxxF19EFT,csTest,csTest,0,0,dNdE);
   for(int i=0;i<100;i++) dnde[i]/=dNdERecoil( 0.4*(i+0.5)+4, dNdE);
   displayPlot("SD/SI","E[keV]",4,44, 0,1 ,"",100,dnde,NULL);    
