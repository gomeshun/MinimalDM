...
double CRESST_III_90_fig7(double M)
{
// 1904.00498 Fig.7
  double X[]={252,   259.5, 267,  283.5, 307.5, 343.5, 384, 448.5, 516, 604.5, 697.5, 784.5, 831,   877.5, 946.5, 1008, 1096.5, 1146.0,1204.5, 1249.5   };
  double Y[]={271.5, 316.5, 355.5,396,   455.5, 490.5, 522, 546,   570, 601.5, 634.5, 655.5, 676.5, 696,   727.5,  756,  775.5,  792,   814.5,  831    };
 
  double  x= 696+log10(M)*(1255.5-696);
  if(x>X[19] || x<X[0])  return sqrt(-1);
  double  y=polint3(x,20,X,Y);
  return  1E-36*pow(10, 3*(y-508.5)/(327-508.5));             
}

double CRESST_III_90(double M)
{
// 1904.00498  Fig.8
  double X[]={231, 237,  253.5, 276, 321,  442.5,568.5,670.5,724.5, 804, 861,942, 1008, 1089,  1176,  1254 };
  double Y[]={94.5,169.5,244.5, 318, 394.5,469.5,541.5,585,  621,   678, 711,742, 786,   823.5, 844.5,847.5};
 
  double  x= 591+log10(M)*(1041-591);
  if(x>X[15] || x<X[0])  return sqrt(-1);
  double  y=polint3(x,16,X,Y);
  return  1E-36*pow(10, 3*(y-394.5)/(394.5-621));             
}
...
{
// 1904.00498
  double X[]={ 225,   274.5, 330,   433.5, 535.5, 673, 757.5, 825,   943.5, 1060, 1144.5, 1255.5 };
  double Y[]={ 289.5, 364.5, 421.5, 454.5, 489.5, 528, 538.5, 550.5, 550.5, 543,  534,      523.5  };
  double  x= 591+log10(M)*(1041.5-591);
  if(x>X[11] || x<X[0])  return sqrt(-1);
  
  double  y=polint3(x,12,X,Y);
  return  1E-36*pow(10, 5*(y-696)/(471-696));             
}
...
{
//     return dNdERecoil(E,dNdE)* exp(-pow((eCresst-E)/0.004,2)/2)/0.01; 
//  return dNdERecoil(E,dNdE)* exp(-pow((eCresst-E)/0.006,2)/2)/0.014;
  return dNdERecoil(E,dNdE)* exp(-pow((eCresst-E)/0.003,2)/2)/(2.5*0.003);
}
...
{ 
   ...
   *N=0;
   ...
   {  if(events[l]+0.00005<events[l+1])        
      { double cc=3.64*simpson_arg((funcArg)dNdERecoil,dNdE, events[l]+0.00005,events[l+1],1E-2,NULL);
             if(cc>m) {m=cc;lm=l;}     
     }
     (*N)++;
...
{ 
   ...
   for(int l=0;l<442;l++) prob[l+1]=3.64*simpson_arg((funcArg)dNdERecoil,dNdE, events[l],events[l+1],1E-2,NULL);
...
static int  initDDexpCS( unsigned int Experiments, double(*vfv)(double),double cs_SI_P, double cs_SI_N,  double cs_SD_P, double cs_SD_N)
{
    ...
    SxxType *sdF19=SxxF19SHELL, *sdXe129=SxxXe129SHELL, *sdXe131=SxxXe131SHELL;      
    if(spinDepFlag==EFT)   { sdF19=SxxF19EFT,  sdXe129=SxxXe129EFT, sdXe131=SxxXe131EFT;  }

    if((Experiments &XENON1T_2018) && maxRecoil(131)>1.69  )
    {
      nucleusRecoilCS(vfv,131,Z_Xe,J_Xe131,sdXe131,cs_SI_P, cs_SI_N,  cs_SD_P, cs_SD_N,dNdE_); 
      for(int i=0;i<RE_DIM;i++) dNdE[i]=0.218*dNdE_[i];
      nucleusRecoilCS(vfv,129,Z_Xe,J_Xe129,sdXe129,cs_SI_P, cs_SI_N,  cs_SD_P, cs_SD_N, dNdE_);
    ...
    if((Experiments & PICO_2019) && maxRecoil(19)>2.45+PICOdTr  )
    {
      ...
      double  xC=1-xF;    
      nucleusRecoilCS(vfv,19,Z_F,J_F19,sdF19,cs_SI_P, cs_SI_N,  cs_SD_P, cs_SD_N, dNdE);      
...
*/      
     ...
     {    
        ...
        {  eCresst=RE_START*pow(RE_STEP,i);
           double Emin=eCresst-0.009;
           if(Emin<RE_START) Emin=RE_START;
           dNdE_CRESST_2019[i]=simpson_arg(CRESST_smooth, dNdE ,Emin,eCresst+0.009,1E-2,NULL);
        }
//char mess[30];
//sprintf(mess,"Mcdm=%e\n",Mcdm);        
...
static int  initDDexp( unsigned int Experiments, double(*vfv)(double))
{
    ...
    SxxType *sdF19=SxxF19EFT,  *sdXe129=SxxXe129EFT, *sdXe131=SxxXe131EFT;
        
    if(spinDepFlag==SHELL)      { sdF19=SxxF19SHELL,  sdXe129=SxxXe129SHELL,  sdXe131=SxxXe131SHELL;  }
//    else if(spinDepFlag==SHELLm){ sdF19=SxxF19SHELLm; sdXe129=SxxXe129SHELLm; sdXe131=SxxXe131SHELLm; }
//    else if(spinDepFlag==SHELLp){ sdF19=SxxF19SHELLp; sdXe129=SxxXe129SHELLp; sdXe131=SxxXe131SHELLp; }   
    
    if((Experiments &XENON1T_2018) && maxRecoil(131)>1.69 )
    {
      nucleusRecoil(vfv,131,Z_Xe,J_Xe131,sdXe131,dNdE_); 
      for(int i=0;i<RE_DIM;i++) dNdE[i]=0.218*dNdE_[i];
      nucleusRecoil(vfv,129,Z_Xe,J_Xe129,sdXe129,dNdE_);
      for(int i=0;i<RE_DIM;i++) dNdE[i]+=0.262*dNdE_[i];
    ...
    if((Experiments & PICO_2019) && maxRecoil(19)>2.45+1.6 )
    {
      double  xF=8*19./(8*19.+3*12);
      double  xC=1-xF;    
      nucleusRecoil(vfv,19,Z_F,J_F19,sdF19,dNdE);       
